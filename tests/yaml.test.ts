...
async function Setup(adapter: string): Promise<any> {
  const adapterOptions = {
    ...
    devLogs: { enable: false, path: `./tests/${adapter}/logs` },
    secure: {
      enable: true,
      secret: "versedb",
    },
...
describe("YAML", () => {
  ...
  test("updateMany method should update data in the specified file", async () => {
    ...
    const data = [{ name: "John" }, { name: "Jane" }];
    const query = { name: "John" };
    const newData = { $set: { name: "Mike" } };
    const dataname = "updateMany";

    // Act
    await db.add(dataname, data);
    const result = await db.updateMany(dataname, query, newData);
