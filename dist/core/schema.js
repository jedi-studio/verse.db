...
class Schema {
     ...
     */
    validate(data, existingData = null // Optional parameter with a default value
    ) {
        const errors = {};
        for (const field in this.fields) {
            ...
            if (fieldConfig.required && (value === undefined || value === null)) {
                ...
                errors[field] = `Invalid type. Expected ${fieldType}, got ${typeof value}.`;
            }
                ...
                errors[field] = `Must be at least ${fieldConfig.minlength} characters long.`;
            }
            else if (fieldType === "string" &&
                fieldConfig.maxlength &&
                typeof value === "string" &&
                value.length > fieldConfig.maxlength) {
                errors[field] = `Must be at most ${fieldConfig.maxlength} characters long.`;
            }
                ...
                const hasDuplicate = existingData.some((item) => item[field] === value);
                if (hasDuplicate) {
