...
class jsonAdapter extends events_1.EventEmitter {
    constructor(options) {
        super();
    ...
    async add(dataname, newData, options = {}) {
        try {
            ...
            const duplicates = flattenedNewData.some((newItem) => currentData.some((existingItem) => {
                ...
                return (_a = options.uniqueKeys) === null || _a === void 0 ? void 0 : _a.every((key) => {
                    if (Array.isArray(existingItem[key.key]) &&
                        ...
                        return (JSON.stringify(existingItem[key.key].sort()) ===
                            JSON.stringify(newItem[key.key].sort()));
                    }
                    else {
            ...
            fs_1.default.writeFileSync(dataname, JSON.stringify(currentData), "utf8");
    ...
    async drop(dataname) {
        try {
            ...
            const emptyData = [];
            fs_1.default.writeFileSync(dataname, JSON.stringify(emptyData), "utf8");
            (0, logger_1.logSuccess)({
    ...
    async search(dataPath, collectionFilters) {
        try {
            ...
            for (const filter of collectionFilters) {
                ...
                const filePath = path_1.default.join(dataPath, `${dataname}.json`);
                const data = await fs_1.default.promises.readFile(filePath, "utf-8");
                const jsonData = JSON.parse(data);
                let result = jsonData;
                if (Object.keys(query).length !== 0) {
                    result = jsonData.filter((item) => {
